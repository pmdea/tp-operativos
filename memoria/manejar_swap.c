...
void* crear_swap(uint32_t size, uint32_t pid){
	...
	log_info(logger, "****CREANDO SWAP PID %d NAME %s****", pid, swap_name);
	...
	log_info(logger, "****SWAP PID %d CREADO EXITOSAMENTE****", pid);
	return swap_process_mem;
}

void eliminar_swap(uint32_t pid, void* swap, uint32_t size){
	char* file = generar_filename(pid);
	log_info(logger, "****BORRANDO SWAP PID %d NAME %s****", pid, file);
	munmap(swap, size);
	remove(file);
	free(file);
	log_info(logger, "****SWAP PID %d BORRADO EXITOSAMENTE****", pid);
}
