...
void enviar_paquete(int socket, FILE* archivo, int *tam_proceso) {
	...
	int tamanio_instrucciones = sizeof(int) //tam proceso
			+ cant_instrucciones*sizeof(ID_INSTRUCCION) //identificadores
			+ cant_parametros_leidos*sizeof(int); //parametros

	int tamanio_total = tamanio_instrucciones
			+ sizeof(int) //este mismo
			+ sizeof(op_code); //operacion

	void* envio = malloc(tamanio_total);
	int desp = sizeof(op_code);
	op_code cod = ENVIO_PROCESO;

	memcpy(envio, &cod, sizeof(op_code));

	memcpy(envio + desp, &tamanio_instrucciones,sizeof(int));
	desp += sizeof(int);
	memcpy(envio + desp, tam_proceso, sizeof(int));
	desp += sizeof(int);
	...
	while (fgets(linea,14,archivo) != NULL) {
		...
		if (id == NO_OP) {
			...
			for(int i = 0; i < cantidad_de_parametros(id); i++) {
				...
				memcpy(envio,&parametro,sizeof(int));
				desp += sizeof(int);
			}
		}
