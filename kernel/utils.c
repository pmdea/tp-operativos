...
void enviarStringSerializado(char* mensaje, int socket){
	int tamanioTexto = strlen(mensaje) + 1;
	int tamanioMensaje = sizeof(int) + tamanioTexto;

	void* buffer = asignarMemoria(tamanioMensaje);

	int desplazamiento = 0;
	concatenarString(mensaje, &desplazamiento, mensaje);
	enviarMensaje(socket, buffer, tamanioMensaje);
	free(buffer);
}
...
pcb* deserializarPCB(int socket_kernel){
	...
	log_info(logger, "He recibido un proceso");
	return unPCB;
}
